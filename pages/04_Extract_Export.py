...
with tab1:
    ...
    st.markdown("The AI will detect entities, fields, and relationships from your documents.")
    ...
    if docs:
        ...
        with col2:
            top_k = st.number_input("Chunks", min_value=1, max_value=50, value=10)
                    ...
                    if result:
                        ...
                        if st.button("📥 Export to Excel", use_container_width=True):
                            try:
                                ...
                                tmp_path = tmp.name
                                tmp.close()
                                pipeline.export_excel(result, tmp_path)
...
with tab2:
    ...
    if docs2 and st.button("Extract with Schema", type="primary", use_container_width=True):
        ...
        for f in st.session_state["schema_fields"]:
            if f["name"].strip():
                schema[f["name"].strip()] = f["desc"].strip()
                    ...
                    try:
                        ...
                        with open(tmp_path, "rb") as f:
                            st.download_button(
                                "📊 Download Excel File",
                                data=f,
                                file_name="extracted_schema.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            )
                    finally:
                        os.unlink(tmp_path)
...
st.markdown("""
```
Upload Document → OCR (if scanned) → Chunking → Embedding → Vector Search
                                                                ↓
                                          LLM Extraction → JSON → Excel
```
""")
